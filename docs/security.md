...
- repeated loop entry reuses only compiler-owned direct and control storage for
  the enclosing function invocation; branch-specific target slots and safe-
  string array-branch classification cannot be conflated to expose or discard
  callable authority in a later `else` body;
