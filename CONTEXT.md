...
This is the detailed normative context for agents and maintainers changing the
Nunjitsu implementation. Human-facing introductions and guides live in
[`README.md`](README.md) and [`docs/`](docs/index.md); exact internal invariants
belong here so those pages can remain readable.
...
1. Read [`README.md`](README.md), [`docs/index.md`](docs/index.md), and this
   context before changing implementation behavior.
...
3. If a cross-cutting decision must change, update this file and any affected
   human-facing guide in the same change, including the consequences.
...
- `tests/compat/`: shared Nunjucks v3.2.4 cases, provenance, manifest, and
  upstream license.
- `docs/`: concise human-facing architecture and maintenance guides.
...
- Keep `README.md` focused on the package introduction, installation, primary
  TypeScript API, minimal contributor workflow, and links into `docs/`.
- Keep `docs/` optimized for human readers: explain concepts, supported use,
  operational workflows, and important tradeoffs without enumerating internal
  regression cases or implementation invariants.
- Keep exact architecture, security, parser, evaluator, compatibility, and test
  invariants in this file. Do not duplicate them into human guides.
- Add a focused human guide only when no existing page has clear ownership, and
